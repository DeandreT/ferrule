...
| --- | --- | --- |
| XML | XSD subset, local include/import graphs, named model/attribute groups, typed element/simple-content/attribute defaults, expanded-name identity for elements and attributes, simple and ordered mixed content, `xsi:nil`, recursive optional/unbounded `##local` skip wildcards, bounded cross-namespace substitution groups, and compatible transitive `complexContent` extension/restriction alternatives | Remaining derived-type input shapes and namespace-aware or validating wildcards |
