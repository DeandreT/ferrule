...
undeclared schemas with explicit non-null `type: object` also normalize an `if` that tests exactly one
required property's presence, an absent or `true` `else`, and a supported
`then` predicate into this same dependent-schema model. Canonical export writes
`dependentSchemas`. Value-sensitive, multi-trigger, general-`if`, and
nontrivial-`else` conditionals, `patternProperties`, unevaluated keywords, and
heterogeneous positional array schemas remain explicit rejections rather than
approximations.
