...
Active JSON Schema `contains` validation, including modern
`minContains`/`maxContains` modifiers, now rejects explicitly rather than
silently accepting unsupported array semantics. Draft 4 resources and legacy
`$ref` siblings retain their dialect-defined ignore behavior.
