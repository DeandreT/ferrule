...
fn render_source(program: &Program) -> Result<String, EmitError> {
    let mut source = String::from(
        "// Generated by ferrule. Do not edit.\n\
         use codegen_runtime::{\n\
             Instance, RuntimeError, ScalarType, Value, adapt_target_value, call, field, group,\n\
             repeated, require_bool, resolve_scalar, scalar,\n\
         };\n\n",
    );
    source.push_str(
    ...
    let body = match expression {
        Expression::SourceField { path } => {
            let path = path
                ...
                .collect::<Vec<_>>()
                .join(", ");
            format!("resolve_scalar(source, &[{path}]).map_err(RuntimeError::from)")
        }
        ...
        Expression::Call { function, args } => {
            ...
            for arg in args {
                body.push_str(&format!("        args.push(expression_{arg}(source)?);\n"));
            }
        ...
        Expression::If {
            ...
            "{{\n        let condition = expression_{condition}(source)?;\n        if require_bool({condition}, condition)? {{\n            expression_{then}(source)\n        }} else {{\n            expression_{else_}(source)\n        }}\n    }}"
        ),
    };
    Ok(format!(
        "fn expression_{id}(source: &Instance) -> Result<Value, RuntimeError> {{\n    let _ = source;\n    {body}\n}}\n\n"
    ))
}
...
fn render_scope(
    ...
    let mut output = format!("fn {name}(source: &Instance) -> Result<Instance, RuntimeError> {{\n");
...
mod tests {
    ...
    use super::*;
    use codegen::{Binding, ExpressionNode, ScalarFunction};
    use ir::SchemaNode;
